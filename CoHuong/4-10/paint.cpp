...
using namespace std;
void file() {
  const string FILE_NAME = "paint";
  freopen((FILE_NAME + ".inp").c_str(), "r", stdin);
  freopen((FILE_NAME + ".out").c_str(), "w", stdout);
}
using ll = long long;
const int oo = 2e9;
const int N = 13, M = 1000;
int a[N][M], dp[M][1 << N];
int n, m;
int call(int i, int mask) {
  ...
  for (int b = 0; b <= 1; ++b) {
    int nxt = b << 0;
    for (int j = 1; j < n; ++j) {
      if (((mask >> (j - 1) & 1) + (mask >> j & 1) + (nxt >> (j - 1) & 1)) & 1)
        nxt ^= 0 << j;
      else
        nxt ^= 1 << j;
    }
    res = max(res, call(i-1, nxt));
  }
  return dp[i][mask] = sum + res;
}
int main() {
