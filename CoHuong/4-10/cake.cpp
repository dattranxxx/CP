                      /*.........      .........               
                  ......  ..   ..  ..  ..      ... .           
                ...   ..  ..........'...,....  ..   ..      .. 
              .'.   ........'',,,,,,,,,,''.......    'l: .,;,;;
            ...  .....'',;::::::::::cccccc;........... .;ccc:;;;.
           .. ......,::::c:ccc:::::cccc:;'.',;;,..''...  .dc:;;;,;.
          ';. ..,,;::c:::c::::::::::cc;'.';;;;;;::'.'.    llc:::::;.
         ;;...,,,:cccc:cccc:::cc:::,',:;;;;;;;;;;;:;..... .l::;ccc:;.
        ;:  .,,'';:;c;:::cc::::;'',;,;;;::;;;;;;:::c:.'.   .oc::ccc;;.
       .c; .;,;;;:';:':';;;;'';;,,;;;;;;;::;::::::::::..   .occcc:::c;,
       :cl.:;,;::;;:c;::;;;;;;;;:;;;;;;::::::::c:cc:cc:... 'docclc::; .'.
      'cco;c,;;:;;;:c;::;;;;;;;;:c::::::::::::::::::;cc:;;;ldolccl:::.
      ;c:lc':::;;;;co::cc:;:;;:::lc:ccc:::::c::::::cl;co:ccoxooc:cc:,'
      c::lo,::c:::;lxocccc:::::::cd::::c:::::cc::cc:cc:lc:;cdloc::c:;,
     ,c;:ol:cccc:::oc;c:::cc:::::ccc;;::::::::cll:;;:ccloc::dooc;:cc;,.
    'll;:dc:::cc::cd,..:::::cc::;;c::..',,:cc:ccc:..;cc;..:;dxoc;:cc;;;'
    c.':cl::::cc::co.  .';;:;;:::;:oc:..:kKNNXX0Odc;;oo:c:c:dxd:;;cl::;,.
   .: .ccl::;;:c:;cc .....';;;;;;,;:.',''cX00kK0lxo.:lc.;,.cdol,,;cc:cl;,.
    c ':cl:x:;:c;;;;;kOOKKxl.'c;;;;;,. . o0OOkO0..;;:c:;c'.:do;,,;c:ccol,,.
    .;c::lldo;;:;;,dO,.O00OO0;....,,',.  .oocldl .,,:c;;::..oc,,,,:;ccc.c,;
     :;;ldldd;;;:,,l:  oKOOxkx         .....''....,,lc:;cc' :,,;,;;c;c' c,c.
   .'::olocoocc:;:,;:.  :od:c,.        ..........';;:;:;ll,.:,,;,,;:::. c;c.
 ...,:lx:l:oclodl:c;;;....'.....       ...........',,;::co::,,c;;;lc,:' o;:
    ,:lo:;::cldodldooo:..........       ..........,,,;::x:,c,;d;,:ll::ox;.
    ;;oc..;;cccoodxxl:c..........                .l,;;::;; l;cl:':xcoc,,;.
    ,,l:l. :;,:lododc;cc.......     .  ..       .;',:;:.c. ld:,o,':..,l,,:.
    .,:. '. ,,,oloooc,:cl' .                  ..,cc,:;c'.  :..,;c:,,,..l,;l.
       ..  ..',ccdolc,,cldo'..              .',,:c,,;:c.         .''.,::;cl.
            '',oodoo::,llol  . .....     .',,;:;:c,,;c,         ..,;....
         .',;,:,.:dlo:;lloc         :,,;:;,;:c:cc:':ccd,...  ..,...,'
        .',;'. .;odlo,;:lxl;'.  ..;cc;:,;::,...;:,,o:lk:'',,''......;.
                 ,oo;',:loo;;;;;'xNo..ldd,.   .:'';ock;;;,,,'.   ...cO,
                .,,;'',,l;oc,'''cXX; .k00c    .;,''x0l''',','.......'0Xx
                ..;''''':..',,''dKXl :0OOk'   .;oc';k,'',..  ........kXXc
                dc;.,,'',:,'..''k0Ok,k0OOOOc. .dkKdo;:;;.    ........KX0x
               l0c..';'',;;,''.,OOOOOOOOOOO0koO000Kk.....'.........lkXOkO.
               OK,    ...;;.. .l0OOOOOOOOOOOOOOOO0Kl   ..'.....  .;XXXOxk'
              ;OKk.......,..   xKOOOOOOOOOOOOOOOO00;   ..    ...;XXK000kO.
              okOo       ... ..0OOOOOOOOOOOOOOOOOOk.   .... ....;X0O00OkO.
              xO0d.     .''. ..KOOOOOOOOOOOOOOOOOOx.... ....,:..dKOOOOOOO
              x000:..... .... .kOOOOOOOOOOOOOOOOOOl........:xo,dKOOOOOOkk
              kO0Kx,;'    ....;',xkklc..::,:ocodoO,........cXXK0OOOOOOOkO'
             .O000000'    ...,......'..........' '....'.....xK000OOOOOOOk
             ,kkOOO00.. .''.. .. ..................  ......,k0O00000OOO*/
#include <bits/stdc++.h>
#define print(_v) for (auto &_ : _v) {cerr << _ << ' ';} cerr << endl;
using namespace std;
void file() {
  const string FILE_NAME = "cake";
  freopen((FILE_NAME + ".inp").c_str(), "r", stdin);
  freopen((FILE_NAME + ".out").c_str(), "w", stdout);
}
using ll = long long;
const int oo = 1e9;
const int N = 50;
int a[N*2+1], dp[N*2+1], f[N*2+1];
int n;
struct fenwick_tree {
  int bit[N * 2 + 1];
  void reset() {
    memset(bit, 0, sizeof(bit));
  }
  void update(int i, int x) {
    for (; i <= 2*n; i = i | (i + 1))
      bit[i] += x;
  }
  int get(int i) {
    int res = 0;
    for (; i >= 0; i = (i & (i + 1)) - 1)
      res += bit[i];
    return res;
  }
  int get(int l, int r) {
    if (l == 0) return get(r);
    return get(r) - get(l-1);
  }
} bit;
int check(int L, int R) {
  // chia dc a thanh it nhat 2 doan sao cho tong moi doan nam trong doan [L, R]
  int res = 0, lo, hi, mid, l, r;
  for (int k = 1; k <= n; ++k) {
    // [r, r + n]
    memset(dp, 0, sizeof(dp));
    bit.reset(); bit.update(k-1, 1);
    for (int i = k; i <= k + n - 1; ++i) {
      lo = k, hi = i, l = -1;
      while (lo <= hi) {
        mid = (lo + hi) >> 1;
        if (f[i] - f[mid-1] <= R) {
          l = mid;
          hi = mid - 1;
        } else {
          lo = mid + 1;
        }
      }
      lo = k, hi = i, r = -1;
      while (lo <= hi) {
        mid = (lo + hi) >> 1;
        if (f[i] - f[mid-1] >= L) {
          r = mid;
          lo = mid + 1;
        } else {
          hi = mid - 1;
        }
      }
      if (l == -1 || r == -1)
        continue;
      if (i == k + n - 1)
        bit.update(0, -1);
      if (bit.get(l-1, r-1)) 
        dp[i] = 1, bit.update(i, 1);
    }
    if (dp[k + n - 1]) 
      return 1;
  }
  return 0;
}
int main() {
  cin.tie(0)->sync_with_stdio(0); cout.tie(0);
  file();
  cin >> n;
  for (int i = 1; i <= n; ++i)
    cin >> a[i], f[i] = f[i-1] + a[i];
  for (int i = n + 1; i <= n * 2; ++i)
    a[i] = a[i - n], f[i] = f[i-1] + a[i];
  vector<int> s; s.reserve(n * n);
  for (int i = 1; i <= n; ++i) 
    for (int j = i; j <= i + n; ++j)
      s.emplace_back(f[j] - f[i-1]);
  sort(s.begin(), s.end());
  s.erase(unique(s.begin(), s.end()), s.end());
  int res = oo;
  for (int i = 0; i < s.size(); ++i) {
    int lo = i, hi = s.size() - 1, mid;
    while (lo <= hi) {
      mid = (lo + hi) >> 1;
      if (check(s[i], s[mid])) {
        res = min(res, s[mid] - s[i]);
        hi = mid - 1;
      } else {
        lo = mid + 1;
      }
    }
  }
  cout << res;
  return 0;
}

