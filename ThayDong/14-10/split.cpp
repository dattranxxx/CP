...
int n, m, x, y, z;
int timer = 1, ans = -1;
int low[N], tin[N], used[N * 2], col[N];
int dfs(int u) {
  int siz = 1;
  low[u] = tin[u] = timer++;
  for (C& c : G[u]) if (!used[c.i]) {
    used[c.i] = 1;
    if (!tin[c.v]) {
      int ns = dfs(c.v);
      siz += ns;
      ...
      if (low[c.v] > tin[u]) {
        cerr << u << ' ' << c.v << ' ' << siz << '\n';
        if (ns == n - x || ns == n - y || ns == n - z) ans = c.i;
      }
        ans = c.i; 
    } else {
...
int main() {
  ...
  // tim cau sao cho chia dc thanh 1 phan a/b/c, mot phan tong 2 cai con lai
  // sau dop pick leaf
  dfs(0);
  cout << ans;
  return 0;
