...
const int oo = 1e9;
const int N = 1500;
int a[N+1], b[N+1], dp[N+1][N+1], g[N+1][N+1];
vector<int> G[N];
int n, m;
int main() {
  ...
  while (T--) {
    ...
    for (int i = 1; i <= m; ++i)
      cin >> b[i];
    // a[i] == b[j] -> dp[i][j] = max(dp[u][v]) + 1 : a[u] == b[v], u < i, v < j, a[u] * 2 <= a[i];
    // -> for u : a[u] * 2 <= a[v] : max(dp[u][v]) : v [0, j-1];
    // : co it cap (u, i) ma a[u] * 2 <= a[i]
    int res = 0;
    memset(g, 0, sizeof(g));
    memset(dp, 0, sizeof(dp));
    for (int i = 1; i <= n; ++i) 
      for (int j = i-1; j; --j) 
        if (a[j] * 2 <= a[i]) G[i].emplace_back(j);
    for (int i = 1; i <= n; ++i) {
      g[i][0] = 0;
      for (int j = 1; j <= m; ++j) {
        if (a[i] == b[j]) {
          dp[i][j] = 1;
          for (int u : G[i]) dp[i][j] = max(dp[i][j], g[u][j-1] + 1);
          res = max(res, dp[i][j]);
        }
