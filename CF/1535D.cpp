...
struct segment_tree {
  int k, p, root;
  string s;
  vector<int> L, R, P, st; char ch;
  segment_tree(int k, string s): k(k + 1), s(s), L(1 << (k + 1)), R(1 << (k + 1)), st(1 << (k + 1)), P(1 << (k + 1)) { 
    build();
  }
  void calc(int i) {
    char c = s[i - (1 << (k - 1))];
    if (c == '0') st[i] = st[L[i]];
    else if (c == '1') st[i] = st[R[i]];
    else st[i] = st[L[i]] + st[R[i]];
  }
  void dfs(int i, int l, int r) {
    if (i <= (1 << (k - 1)))
      return st[i] = 1, void();
    int m = (l + r) >> 1;
    dfs(L[i], l, m), dfs(R[i], m+1, r);
    P[i - (1 << (k - 1))] = m;
    calc(i);
  }
  void build() {
